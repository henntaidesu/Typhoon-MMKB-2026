...
code(s, 0.75, 1.32, 7.55, 5.35, [
    ...
    "    ON secondary_disaster FOR EACH ROW",
...
code(s, 0.75, 5.05, 7.55, 1.65, [
    ...
    "--   本番は embed('造成严重洪水和滑坡的强台风') を束縛。ここは既存ベクトルで代用",
...
code(s, 8.55, 3.6, 4.1, 3.1, [
    "-- 実行結果 (実測)",
    "SELECT fn_region_landfall_count(3);",
    "-- => 600   China (国)",
    "",
    "SELECT fn_region_landfall_count(173);",
    "-- => 217   広東省",
    "",
    "-- 省別 (実測)",
    "-- 広東 217 / 海南 121",
...
code(s, 0.75, 1.32, 6.9, 4.05, [
    ...
    "WITH q AS (SELECT embedding AS qvec FROM typhoon   -- 本番は",
    "            WHERE intl_id = '9922')      -- embed(...) の結果を束縛",
